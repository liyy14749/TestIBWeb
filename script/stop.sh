...
do
  ...
  if [ $num -gt 0 ]; then
    ...
    echo "[${CUR_TIME}]-${PROJECT_NAME_JAR}: to stop." >> "${PROJECT_CONSOLE_LOG_PATH}/${client_host}.${PROJECT_NAME}.log"
  else
    echo "[${CUR_TIME}]-${PROJECT_NAME_JAR}: not running." >> "${PROJECT_CONSOLE_LOG_PATH}/${client_host}.${PROJECT_NAME}.log"
  fi
done